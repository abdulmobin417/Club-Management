<?php
  ...
  $eventQuery = "SELECT * FROM clubevent WHERE status = 'approved';";
  ...
  <body>
    ...
    <section>
      <div class="flex items-center font-poppins mb-16">
        <div class="justify-center max-w-6xl px-4 py-16 mx-auto">
          <h2 class="text-4xl font-bold text-center text-teal-500">
            All Events
          </h2>
          <div class="w-24 mx-auto mb-8 border-b border-red-700"></div>
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-3 md:grid-cols-2">
          <?php
            if($createEvent->num_rows>0){
              while($row = mysqli_fetch_assoc($createEvent)){
                $clubId = $row['clubId'];
                ...
                $postUserQuery = "SELECT * FROM users WHERE userId = $postUserId;";
                $createPostUserQuery = mysqli_query($conn, $postUserQuery);
                $postUserData = mysqli_fetch_array($createPostUserQuery);
                if(strlen($row['eventDescription'] > 400)){
                  $description = substr($row['eventDescription'],0,100).'.....'; 
                }
          ?>
            <div class="relative shadow-md">
              ...
              />
              <a href="../clubDetails/index.php?clubId=<?php echo $clubId; ?>" class="text-sm text-white">
                <div class="absolute top-0 p-2 m-2 bg-red-600">
                  <?php echo $clubData['clubName']; ?>
                </div>
              </a>
              <a href="../eventDetails/index.php?eventId=<?php echo $row['eventId']; ?>" class="p-3 block">
                <div class="mb-2 text-xs text-gray-600">
                  Posted by 
                  <span class="font-bold">
                    <?php echo $postUserData['FirstName'].' '.$postUserData['LastName'];?>
                  </span>
                </div>
